...
quant_methods = c("ge","exon","tx", "txrev")
...
quant = dplyr::tibble(quant = quant_methods, matrix_suffix = matrix_suffix, meta_file_name = meta_file_name, c = 1)
susie_file = dplyr::left_join(qtl,quant, by = "c") %>%
  ...
  dplyr::select(study, qtl_group, expression_matrix, phenotype_meta, sample_meta, vcf, phenotype_list, covariates)
