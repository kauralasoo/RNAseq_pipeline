...
sample_metadata = readr::read_tsv("../SampleArcheology/studies//cleaned/Fairfax_2014.tsv")
...
#Export normalized expression data to disk
norm_mat = round(assays(fairfax_2014_norm)$norm_exprs,3)
write.table(norm_mat, "results/expression_matrices/HumanHT-12_V4/Fairfax_2014.normalized.tsv", sep = "\t", quote = F)
...
fairfax_2012_norm = filterSummarizedExperiment(fairfax_2012_se, filter_rna_qc = TRUE, filter_genotype_qc = TRUE) %>%
  array_normaliseSE(norm_method = "quantile", assay_name = "exprs", log_transform = TRUE, adjust_batch = TRUE)
saveRDS(fairfax_2012_norm, "results/SummarizedExperiments/array_norm/Fairfax_2012.rds")


#### CEDAR ####
gene_meta = readr::read_tsv("metadata/gene_metadata/HumanHT-12_V4_gene_metadata.txt.gz")
sample_metadata = readr::read_tsv("../SampleArcheology/studies/cleaned/CEDAR.tsv")
