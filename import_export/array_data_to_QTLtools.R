...
expression_matrix = read.table("results/expression_matrices/HumanHT-12_V4/Fairfax_2014.tsv.gz", sep = "\t", check.names = FALSE)
...
#Filter SE to keep correct chromosomes and QC-passed samples
se_filtered = filterSummarizedExperiment(cedar_se, valid_chromosomes = valid_chromosomes, filter_rna_qc = F, filter_genotype_qc = F)
...
sample_metadata = readr::read_tsv("metadata/cleaned/GENCORD.tsv")
expression_matrix = read.table("results/expression_matrices/featureCounts/GENCORD.tsv.gz", sep = "\t")
featureCounts_se = makeSummarizedExperiment(expression_matrix, gene_meta, sample_metadata, assay_name = "counts")

#Filter and normalise data for QTL mapping
qtltools_se = qtltools_prepareFeatureCountsData(featureCounts_se)
